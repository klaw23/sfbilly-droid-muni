...
abstract class Parser {
	public void parse(InputStream input) {
		...
		try {
			...
			if ("Error".equals(parser.getName())) {
				...
				Log.w("DroidMuni", "Unexpected body element: "
						+ parser.getName());
				result_state = ResultState.PARSE_ERROR;
			}
		} catch (XmlPullParserException e) {
			Log.d("DroidMuni", "Parse error", e);
			result_state = ResultState.PARSE_ERROR;
		} catch (IOException e) {
	 ...
	 * http://www
	 * .nextmuni.com/s/COM.NextBus.Servlets.XMLFeed?command=routeConfig
	 * &a=sf-muni&r=71'
	 */
	private void parseError() throws XmlPullParserException, IOException {
		...
		if (shouldRetry) {
			Log.w("DroidMuni",
					"Unexpected error document with shouldRetry==true");
